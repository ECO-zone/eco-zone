...
def add_zone_info(apps, schema_editor):
    ...
    for plant in plants:
        print(plant.name)
        records = list(
            TimeseriesRedispatch.objects.filter(
                redispatch__power_plant_id=plant.id
            ).all()
        )
        for record in records:
            record.region_north_south = plant.region_north_south
            record.is_renewable = plant.is_renewable
        TimeseriesRedispatch.objects.bulk_update(
            records, ["region_north_south", "is_renewable"], batch_size=1000
        )
