...
from ecozone.models import get_emissions, Generation, PowerPlant, Redispatch, TimeseriesRedispatch, PSR_TYPES_POST_2024


class ZonalEmissionFactorTests(TestCase):

    @classmethod
    def setUpTestData(cls):
        PowerPlant.objects.update_zone_data()
        start = datetime(2024, 12, 31, 0, 0, 0, tzinfo=timezone.utc)
        end = datetime(2025, 1, 3, 11, 45, 0, tzinfo=timezone.utc)
        generation_records = []
        while start <= end:
            params = {"start": start}
    ...
    def test_emission_factor_is_zero_when_res_redispatch_south(self):
        ...
        end = datetime(2025, 1, 1, 16, 0, 0, tzinfo=timezone.utc)
        for i in self.results:
            if i[0] > start and i[0] < end:
                assert i[1] == 0

    def test_emission_factor_is_national_value_when_no_res_or_con_redispatches_in_south(self):
        """
        ...
        res_start = datetime(2025, 1, 1, 12, 0, 0, tzinfo=timezone.utc)
        res_end = datetime(2025, 1, 1, 16, 0, 0, tzinfo=timezone.utc)
    ...
    def test_emission_factor_is_redispatch_value_when_con_redispatch_in_south(self):
        ...
        """
        start = datetime(2025, 1, 1, 18, 0, 0, tzinfo=timezone.utc)
        end = datetime(2025, 1, 1, 20, 0, 0, tzinfo=timezone.utc)
        for i in self.results:
            if i[0] > start and i[0] < end:
                assert i[1] == 933.0
